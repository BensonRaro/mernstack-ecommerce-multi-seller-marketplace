...
export default function SellerDashboard() {
  ...
  const stats = data;
  const storeName = sellerMe?.seller?.name ?? "your store";
    ...
    <div className="mx-auto max-w-[1200px] space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Welcome back — here&apos;s what&apos;s happening with {storeName}.
        </p>
      </div>

      <SellerStatCards stats={stats} />

      <SellerSalesChart data={stats?.salesOverTime ?? []} />

      <SellerBestSellers bestSellers={stats?.bestSellers} />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {stats?.avgRating ? `${stats.avgRating.toFixed(1)} / 5 from ${stats.totalReviews} reviews` : "No reviews yet"}
                </p>
              </div>
            </div>
